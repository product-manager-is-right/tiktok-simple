...
func TestUserVideo(t *testing.T) {
	configs.ReadConfig(configs.TEST)
	cli, err := client.NewClient()
	if err != nil {
		panic(err)
	...
	status, body, err := cli.Post(context.Background(), nil, "http://tiktok.simple.user/douyin//publish/UserVideo/?userId=24&videoId=24", nil, config.WithSD(true))
	res := vo2.Response{}
	if err = json.Unmarshal(body, &res); err != nil {
		return
	}
